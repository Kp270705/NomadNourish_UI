<script lang="ts">
    ...
    // $: authButton = $isAuthorized ? 'Sign-out': 'Sign-in';
...
<div class="text-center">
  <Button onclick={() => (open2 = true)}>Others</Button>
</div>
  ...
  <Sidebar disableBreakpoints={true} class="top-16">
    <SidebarWrapper class="overflow-y-auto rounded-sm px-3 py-0 dark:bg-gray-800">
      <SidebarGroup>
        ...
        {#if $isAuthorized }
            <div class=" cursor-pointer " onclick={() => promptToCancel() }  >
            <!-- <a use:link href="/"  onclick={() => promptToCancel() }  > -->
